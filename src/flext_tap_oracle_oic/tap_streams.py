...
class _OicEnvelope(BaseModel):
    items: list[mt.ContainerValue] | None = None
    data: list[mt.ContainerValue] | None = None
    total_size: Annotated[int | None, Field(default=None, alias="totalSize")]
    count: int | None = None


def _as_value_list(value: mt.ContainerValue) -> list[mt.ContainerValue] | None:
...
class OICPaginator:
    ...
    def _calculate_next_offset(self, data: t.ContainerValue) -> int | None:
...
class OICBaseStream(FlextMeltanoStream):
    ...
    def _is_empty_result_expected(self, data: t.ContainerValue) -> bool:
